  ...
  <UCard>
    <template #header>
      <div class="flex items-center space-x-3">
        ...
        <h1 class="text-2xl font-bold">Yeni Destek Talebi Oluştur</h1>
      </div>
      <p class="text-gray-500 mt-1 pl-9">Lütfen yaşadığınız sorun veya talebinizle ilgili bilgileri girin.</p>
    </template>
    ...
    <UForm :state="state" :schema="schema" @submit="handleCreateTicket" class="space-y-5">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <UFormGroup label="Kategori" name="category" required>
          <USelectMenu v-model="state.category" :options="ticketCategories" by="_id" option-attribute="name" placeholder="Sorun kategorisi seçiniz..." searchable size="lg" />
        </UFormGroup>
        <UFormGroup label="Öncelik" name="priority" required>
          <USelect v-model="state.priority" :options="priorityOptions" size="lg" />
        </UFormGroup>
      </div>
      <UFormGroup label="Açıklama" name="description" required>
        <UTextarea v-model="state.description" :rows="6" placeholder="Lütfen sorunu olabildiğince detaylı anlatın..." />
      </UFormGroup>
      <UFormGroup label="Ekran Görüntüsü (İsteğe Bağlı)" name="attachment">
        <input type="file" @change="handleFileChange" accept="image/png, image/jpeg, image/gif" class="file-input" />
      </UFormGroup>
...
const state = reactive({ title: undefined, category: undefined, priority: 'Medium', description: undefined, attachment: null });

const ticketCategories = ref([]);
const { data, error } = await useFetch('/api/categories?type=Ticket', { headers: { Authorization: `Bearer ${authStore.token}` } });
if (data.value?.success) { ticketCategories.value = data.value.data; } 
else { toast.add({ title: 'Hata', description: 'Kategori listesi yüklenemedi.', color: 'red' }); }
...
<style scoped>
.file-input { @apply block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer; }
</style>