<template>
  <div class="max-w-5xl mx-auto p-6">
    ...
    <div v-else>
      <h1 class="text-2xl font-bold mb-4">{{ ticket.title || 'Talep Başlığı Yok' }}</h1>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm bg-white rounded border p-4 shadow mb-6">
        ...
        <div><strong>Birim:</strong> {{ ticket.department?.name || 'Yok' }}</div>
        <div><strong>Açan:</strong> {{ ticket.openedBy?.fullName || 'Yok' }}</div>
        <div><strong>Oluşturma:</strong> {{ formatDateTime(ticket.createdAt) }}</div>
        <div><strong>Atanan:</strong> {{ ticket.assignedTo?.fullName || 'Atanmamış' }}</div>
      </div>
      ...
      <div v-if="canEdit" class="space-y-6 bg-white p-4 border rounded shadow mb-8">
        ...
        <div>
          ...
          <button @click="updateStatus" class="mt-2 bg-blue-600 text-white px-4 py-2 rounded" :disabled="isSubmitting">
            Durumu Kaydet
          </button>
        </div>
        ...
        <div>
          ...
          <button @click="assignAgent" class="mt-2 bg-green-600 text-white px-4 py-2 rounded" :disabled="isSubmitting">
            Atamayı Kaydet
          </button>
        </div>
        ...
        <div>
          ...
          <textarea v-model="newComment" class="border rounded w-full p-2" rows="3" placeholder="Yorum yaz..."></textarea>
          <button @click="addComment" class="mt-2 bg-gray-800 text-white px-4 py-2 rounded" :disabled="isSubmitting">
            Yorumu Gönder
          </button>
        </div>
      </div>
      ...
      <div class="bg-white border rounded shadow p-4">
        ...
        <div v-if="ticket.history.length === 0" class="text-sm text-gray-500">Geçmiş yok.</div>
        <div v-for="item in ticket.history" :key="item._id" class="border-b py-3">
...
const getToken = () => process.client ? localStorage.getItem('authToken') : null
const getUser = () => {
...
const fetchITAgents = async () => {
  try {
    const res = await fetch(`http://localhost:3001/api/users?role=Admin,ITAgent`, {
      headers: { Authorization: `Bearer ${getToken()}` }
    })
